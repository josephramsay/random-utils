...
def process(pin, pout=None,test=None):
    ...
    for fin in os.listdir(pin):
        if re.match('.*\.tif+',fin): 
            fout = os.path.join(pout,p,fin)
            rioconvert(os.path.join(pin,fin),fout,test)
...
def main():
    ...
    if args.src == 'test':
        ...
        process(os.path.abspath(args.src),os.path.abspath(args.dst))
